const express = require('express');
const jwt = require('jsonwebtoken');
let books = require("./booksdb.js");
const regd_users = express.Router();

let users = [];

const isValid = (username)=>{ 
    let userNow=users.filter((user)=>{
        return user.username==username
    })
    if(userNow){
        return true;
    }
    else {
        return false;
    }
}

const authenticatedUser = (username,password)=>{ 
    let validUser=users.filter((user)=>{
        return (user.username==username && user.password==password)
    })
    if(validUser.length>0){
        return true
    }
    else{
        return false;
    }
    //returns boolean
//write code to check if username and password match the one we have in records.
}

//only registered users can login
regd_users.post("/login", (req,res) => {
  const username= req.body.username;
  const password=req.body.password;
  if(!username||!password){
    res.status(404).json({message:"Error data"})
  }

  if(authenticatedUser(username,password)){
    let accessToken=jwt.sign({
        data:password
    },"access",{expiresIn:60*60})
    req.session.authorization={accessToken,username}
    return res.status(200).send("User successfully logined in")
  }
  else{
    return res.status(208).json({ message: "Invalid Login. Check username and password" });
  }
});

// Add a book review
regd_users.put("/auth/review/:isbn", (req, res) => {
  if(!req.body.review||!req.params.isbn){
    return res.status(300).json({ message: "Invalid ISBN" });
  }
  else{
    books[req.params.isbn].reviews[req.user.data]=req.body.review
    return res.status(200).json({message:"The review for the book with ISBN "+req.params.isbn+" has been added/updated"})
  }
});

regd_users.delete("/auth/review/:isbn",(req,res)=>{
    if(!req.params.isbn){
        res.status(200).json({ message: "Invalid ISBN" });
    }
    else{
        delete books[req.params.isbn].reviews[req.user.data]
        console.log(books[req.params.isbn].reviews)
        res.status(200).json({message:"Review for the "+req.params.isbn+" posted by user "+req.user.data+" deleted"})
    }
})

module.exports.authenticated = regd_users;
module.exports.isValid = isValid;
module.exports.users = users;
